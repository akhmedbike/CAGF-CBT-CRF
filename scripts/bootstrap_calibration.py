...
def write_markdown(crf_boot: dict, cagf_boot: dict, crf_point: dict,
    ...
    rows = [
        ...
        ("CRF Brier (multiclass, Eq. 3)", crf_point["brier_full"], fmt_ci(cr["brier_full"], 1.0, 3)),
        ...
        ("CAGF Brier (top-1) raw", cagf_point["raw"]["brier_top1"], fmt_ci(cg["raw"]["brier_top1"], 1.0, 3)),
        ("CAGF Brier (multiclass, Eq. 3) raw", cagf_point["raw"]["brier_full"], fmt_ci(cg["raw"]["brier_full"], 1.0, 3)),
        ("CAGF ECE T=2.55 (%)", 100 * cagf_point["T255"]["ece"], fmt_ci(cg["T255"]["ece"])),
        ("CAGF ECE T=2.60 (%)", 100 * cagf_point["T260"]["ece"], fmt_ci(cg["T260"]["ece"])),
        ("CAGF Brier (top-1) T=2.60", cagf_point["T260"]["brier_top1"], fmt_ci(cg["T260"]["brier_top1"], 1.0, 3)),
        ("CAGF Brier (multiclass, Eq. 3) T=2.60", cagf_point["T260"]["brier_full"], fmt_ci(cg["T260"]["brier_full"], 1.0, 3)),
